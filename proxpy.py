...
class request:
    ...
    def get(self, url, headers=None, proxyInfo=None, ignoreErrors=None):
        ...
        try:
            ...
            if ignoreErrors == False: print(f"[ProxPy]: GET request failed with error: {e}")
            return None

    def post(self, url, data=None, headers=None, proxyInfo=None, ignoreErrors=None):
        proxy = self.request_proxy()
        if proxyInfo == True: print("[ProxPy]: Using proxy "+proxy)
        proxies = {'http': proxy, 'https': proxy}
        try:
            ...
            if ignoreErrors == False: print(f"[ProxPy]: POST request failed with error: {e}")
            return None
